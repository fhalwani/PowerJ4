...
public class IMonthPanel extends JPanel {
	...
	public IMonthPanel(Calendar initialDate, Calendar minDate, Calendar maxDate) {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent me) {
				IMonthPanel.this.firePropertyChange("Confirm", null, selectedDate);
			}
		});
		setTableSelection();
	...
	private void changeTableAction(int key) {
			...
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					...
					if (table.getSelectedColumn() == 0) {
						...
						IMonthPanel.this.firePropertyChange("Value", selectedDate, newValue);
						setValue(newValue);
						allow = false;
					}
					if (allow) {
		...
		case KeyEvent.VK_UP:
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					boolean allow = true;
					if (min != null && selectedDate.getTimeInMillis() - min.getTimeInMillis() < LibDates.ONE_DAY * 7) {
						IMonthPanel.this.firePropertyChange("Value", selectedDate, min);
						setValue(min);
						allow = false;
					}
		...
		case KeyEvent.VK_DOWN:
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					boolean allow = true;
					if (max != null && max.getTimeInMillis() - selectedDate.getTimeInMillis() < LibDates.ONE_DAY * 7) {
						IMonthPanel.this.firePropertyChange("Value", selectedDate, max);
						setValue(max);
						allow = false;
					}
	...
	private void setTableSelection() {
		boolean previousChange = programmaticChange;
		programmaticChange = true;
		int days = (int)((selectedDate.getTimeInMillis() - startDate.getTimeInMillis()) / LibDates.ONE_DAY);
