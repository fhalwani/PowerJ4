...
public class IChartBase extends JPanel implements IUpdateNotifier, ComponentListener {
	...
	public IChartBase(Dimension dim) {
		super();
		setSize(dim);
		try {
	...
	public void updateBuffer() {
		...
		if (bi == null
				|| bi.getWidth() != dim.width
				|| bi.getHeight() != dim.height) {
			if (dim.width < 200) {
				dim.width = 200;
			}
			if (dim.height < 200) {
		...
		if (cm != null) {
			...
			Bounds bo = BoundsImpl.create(0, 0, dim.width, dim.height);
